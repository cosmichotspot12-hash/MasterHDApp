...
│
├── proxy.ts                         ACTIVE middleware — Next.js 16 renamed the `middleware` convention to `proxy`; guards /admin/* and /api/admin/*
...
- Admin pages (`/admin/*`) and admin APIs ARE middleware-protected by root-level `proxy.ts` (Next.js 16 renamed the `middleware` file convention to `proxy`; build output shows `ƒ Proxy (Middleware)`). Unauthenticated visits to `/admin/*` redirect to `/admin/login`; unauthenticated `/api/admin/*` calls get 401. Admin RSC pages read data via `lib/admin-data.ts` (direct Supabase) or the admin APIs.
...
1. ~~No middleware protection on admin pages~~ — **NOT AN ISSUE.** In Next.js 16 the `middleware` convention was renamed to `proxy`: root-level `proxy.ts` with `export function proxy` IS active middleware and guards `/admin/*` and `/api/admin/*`. Do NOT rename it to `middleware.ts`.
